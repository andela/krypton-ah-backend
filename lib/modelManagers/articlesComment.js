...
const { ArticlesComments, ArticlesHighlights } = require('../../database/models');
  ...
  static async findAllComment(articleId, limit, offset) {
    const options = {
      ...
      include: [
        ...
        {
          model: ArticlesHighlights,
          attributes: ['highlightedText', 'startIndex', 'endIndex']
        }
      ],
      limit,
      offset
 ...
 */
  ...
  /**
 * @static
 * @param {object} field Culumn(s) to be updated with value(s)
 * @param {object} commentId Id of comment to be updated
 * @returns {*} updatedArticleComment
 * @memberof articleModelManager
 */
  static async updateAllCommentThreads(field = { }, commentId) {
    const updatedArticleComment = await ArticlesComments.update(
      field,
      {
