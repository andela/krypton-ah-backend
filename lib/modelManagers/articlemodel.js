...
const searchAttribute = {
  attributes: ['id', 'title', 'description', 'category', 'numberOfReviews', 'createdAt', 'featuredImageUrl']
};
...
class articleModelManager {
  ...
  * @returns {object} Articles by keyword
  */
  static async filterPopularArticles(limit) {
    const options = {
      include: [includeUser, includeTag, includeReactions, includeComments, includeStats],
      limit,
    };
    options.attributes = searchAttribute.attributes;
    const returnedArticles = await Articles.findAll(options);
