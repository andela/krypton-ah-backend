const chai = require('chai'),
  ...
  validate = require('../../middlewares/bookmarkValidate'),
...
let req = {};
let req1 = {};
describe('Check function that validates Article Bookmark Model', () => {
  ...
  it('should send error message for request body parameter violation', async () => {
    ...
    expect(res.json.firstCall.lastArg).to.haveOwnProperty('message').be.an('object');
    expect(res.json.firstCall.lastArg.message).to.haveOwnProperty('articleId').be.an('array');
    expect(res.json.firstCall.lastArg.message.articleId[0]).to.equal('articleId is required');
    expect(res.json.firstCall.lastArg.message.articleId[1]).to.equal('articleId must be a valid uuid');
  });
